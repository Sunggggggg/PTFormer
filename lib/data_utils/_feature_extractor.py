...
from lib.data_utils._occ_utils import get_motion_occlusions, get_cam_noise
from lib.utils.vis import batch_visualize_preds
from lib.data_utils._img_utils import get_single_image_crop, convert_cvimg_to_tensor, get_single_image

def extract_features(model, occluders, video, bbox, debug=False, batch_size=1, kp_2d=None, dataset=None, scale=1.3):
    '''
    ...
    with torch.no_grad():
        ...
        appear_features = torch.cat(appear_features, dim=0)

    return features.numpy(), appear_features.numpy()

def appear_extract_features(model, video, batch_size=4):
    device = 'cuda'
    if isinstance(video, torch.Tensor) or isinstance(video, np.ndarray):
        video = video
    elif isinstance(video, str):
    ...
    video = torch.cat(
            [get_single_image(image).unsqueeze(0) for image in video], dim=0
        )
    ...
    with torch.no_grad():
        for images in frames:
            ...
            pred = model(images)
            appear_features.append(pred.cpu())
