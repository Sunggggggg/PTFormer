...
def read_data(folder, set, debug=False):
    ...
    for i, seq in tqdm(enumerate(sequences)):
        ...
        for p_id in range(num_people):
            ...
            temp = draw_skeleton(img, j2d[0], dataset='3dpw', unnormalize=False, thickness=2)
            cv2.imshow('img', temp)
            cv2.waitKey(0)
            ...
            ############################
            features, x1, x2, x3, x4 = extract_features(model, None, img_paths_array, bbox,
                                        kp_2d=j2d[time_pt1:time_pt2], debug=debug, dataset='3dpw', scale=1.3)
            dataset['features'].append(features)
            #dataset['layer1'].append(x1)
            dataset['layer2'].append(x2)
            dataset['layer3'].append(x3)
            #dataset['layer4'].append(x4)
...
if __name__ == '__main__':
    ...
    joblib.dump(dataset, osp.join(dir, '3dpw_val_layer_db.pt'))

    dataset = read_data(args.dir, 'test', debug=debug)
    joblib.dump(dataset, osp.join(dir, '3dpw_test_layer_db.pt'))

    dataset = read_data(args.dir, 'train', debug=debug)
    joblib.dump(dataset, osp.join(dir, '3dpw_train_layer_db.pt'))
