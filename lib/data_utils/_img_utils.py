...
from lib.data_utils._occ_utils import occlude_with_objects, paste_over
...
def split_into_chunks_test(vid_names, seqlen, stride, is_train=True, match_vibe=True):
    video_start_end_indices = []
    video_names, group = np.unique(vid_names, return_index=True)
    perm = np.argsort(group)
    video_names, group = video_names[perm], group[perm]
    ...
    for idx in range(len(video_names)):
        ...
        if stride != seqlen:
            ...
            d = start_finish[0][0]
            for j in range(int(seqlen/2)):
                dummy = [0, int(seqlen/2) + j - 1]
                start_finish.insert(j,dummy)
                
            d = start_finish[-1][1]
            for j in range(int(seqlen/2) - 1):
                dummy = [d - (int(seqlen/2) + j), d]
                start_finish.append(dummy)
